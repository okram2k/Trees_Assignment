...
class StarshipEnterprise {
  ...
  findOfficersWithNoDirectReports(values = []) {
    // your solution here
    if (this.rightReport == null && this.leftReport ==null){
  ...
  listOfficersByRank(tree, rankedOfficers = {}) {
    const queue = new Queue();
      ...
      while (node) {
        ...
        rankedOfficers[depth].push(node.officerName); // Add that value from the queue to an array
        if (node.rightReport) {
          queue.enqueue(node.rightReport); // Add the left child to the queue
          
        }

        if (node.leftReport) {
          queue.enqueue(node.leftReport); // Add the right child to the queue
